    ...
    if(isset($_POST['checkboxes'])){
        ...
        foreach($_POST['checkboxes'] as $id){
            ...
            if($bulk_option == 'delete'){
                $bulk_del_query = "DELETE FROM `users` WHERE `users`.`id` = $id";
                mysqli_query($con, $bulk_del_query);
            }
            else if($bulk_option == 'user'){
                $bulk_user_query = "UPDATE `users` SET `role` = 'user' WHERE `users`.`id` = $id";
                mysqli_query($con, $bulk_user_query);
            }
            else if($bulk_option == 'admin'){
                $bulk_admin_query = "UPDATE `users` SET `role` = 'admin' WHERE `users`.`id` = $id";
                        ...
                        <tbody>
                           <?php
                                while($row = mysqli_fetch_array($run)){
                                    ...
                                    //$mon_year = date('M, Y', strtotime($row['date']));
